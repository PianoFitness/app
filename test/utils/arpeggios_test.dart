...
void main() {
  group('ArpeggioDefinitions', () {
    ...
    group('MIDI note generation', () {
      test('should generate correct MIDI notes for C Major arpeggio in octave 4', () {
        final arpeggio = ArpeggioDefinitions.getArpeggio(
          MusicalNote.c,
          ArpeggioType.major,
          ArpeggioOctaves.one,
      ...
      test('should generate correct MIDI notes for D Minor arpeggio in octave 3', () {
        final arpeggio = ArpeggioDefinitions.getArpeggio(
          MusicalNote.d,
          ArpeggioType.minor,
          ArpeggioOctaves.one,
    ...
    group('Full arpeggio sequences', () {
      ...
      test('should generate correct C Major two octave sequence with expected MIDI notes', () {
        final arpeggio = ArpeggioDefinitions.getArpeggio(
          MusicalNote.c,
          ArpeggioType.major,
          ArpeggioOctaves.two,
        ...
        final expectedSequence = [60, 64, 67, 72, 76, 79, 84, 79, 76, 72, 67, 64, 60];
      ...
      test('should generate correct A Minor two octave sequence without large jumps', () {
        final arpeggio = ArpeggioDefinitions.getArpeggio(
          MusicalNote.a,
          ArpeggioType.minor,
          ArpeggioOctaves.two,
        ...
        final expectedSequence = [69, 72, 76, 81, 84, 88, 93, 88, 84, 81, 76, 72, 69];
      ...
      test('should generate correct G Major two octave sequence', () {
        ...
        final expectedSequence = [67, 71, 74, 79, 83, 86, 91, 86, 83, 79, 74, 71, 67];
        
        expect(sequence, equals(expectedSequence));
        expect(sequence.length, equals(13)); // 7 up + 6 down
      });

      test('should verify no large jumps in two-octave sequences (regression test)', () {
        ...
        expect(c5Index, greaterThan(-1), reason: 'C5 should be in the sequence');
        expect(sequence[c5Index + 1], equals(76), reason: 'After C5, the next note should be E5 (76), not E6 (88)');
        
        // Verify the complete ascending portion has no jumps larger than 5 semitones (perfect 4th)
        final ascendingPortion = sequence.sublist(0, 7); // First 7 notes (up to C6)
        for (int i = 1; i < ascendingPortion.length; i++) {
          final interval = ascendingPortion[i] - ascendingPortion[i-1];
          expect(interval, lessThanOrEqualTo(5), 
            reason: 'Ascending interval from ${ascendingPortion[i-1]} to ${ascendingPortion[i]} should not exceed a perfect 4th');
        }
      });
    });
    ...
    group('Edge cases and comprehensive coverage', () {
      ...
      test('should generate consistent results across multiple calls', () {
        ...
        expect(arpeggio1.getFullArpeggioSequence(4), equals(arpeggio2.getFullArpeggioSequence(4)));
      });
