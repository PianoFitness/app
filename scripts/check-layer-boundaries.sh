...
check_layer_imports() {
    ...
    for pattern in "${forbidden_patterns[@]}"; do
        ...
        violations=$(find "$layer_path" -name "*.dart" -type f -exec grep -l "import \"package:piano_fitness/$pattern" {} \; 2>/dev/null || true)
        ...
        if [ -n "$violations" ]; then
            ...
            echo "$violations" | while IFS= read -r file; do
                ...
                grep --color=always "import \"package:piano_fitness/$pattern" "$file" | sed 's/^/    /'
            done
            echo ""
