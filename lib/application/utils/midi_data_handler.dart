...
class MidiDataHandler {
  ...
  /// Parses [packet] and dispatches each resulting event to [onEvent].
  ///
  ...
  static void dispatch(
    MidiInputPacket packet,
    MidiState midiState,
    void Function(MidiEvent) onEvent,
  ) {
    try {
      MidiService.handleMidiData(packet.data, (MidiEvent event) {
        try {
          onEvent(event);
        } catch (e, stackTrace) {
